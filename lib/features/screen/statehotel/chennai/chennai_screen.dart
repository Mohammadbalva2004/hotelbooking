...
class ChennaiScreen extends StatefulWidget {
  const ChennaiScreen({super.key});

  @override
  State<ChennaiScreen> createState() => _ChennaiScreenState();
}

class _ChennaiScreenState extends State<ChennaiScreen> {
  int _selectedOption = -1;
  String selectedPriceRange = '₹1500 - ₹2000';
  ...
  void applyFilters() {
    setState(() {
      ...
      if (selectedPriceRange == '₹1500 - ₹2000') {
        minPrice = 1500;
        maxPrice = 2000;
      } else if (selectedPriceRange == '₹2000 - ₹2500') {
        minPrice = 2000;
        maxPrice = 2500;
      } else if (selectedPriceRange == '₹2500 - ₹3500') {
        minPrice = 2500;
        maxPrice = 3500;
      }
  ...
  // Include all the same methods as Mumbai screen with Chennai-specific data
  ...
  void _showPriceFilter(BuildContext context) {
    showCommonBottomSheet(
      ...
      child: StatefulBuilder(
        builder: (context, setState) {
          final priceDistribution = [90, 110, 70];
          return Padding(
            padding: const EdgeInsets.all(16.0),
            child: Column(
              mainAxisSize: MainAxisSize.min,
