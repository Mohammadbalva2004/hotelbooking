...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadFavorites() async {
    ...
    setState(() {
      _favoriteHotels =
          favorites.map((hotel) => hotel['name'].toString()).toSet();
    });
  }

  Future<void> _toggleFavorite(Map<String, dynamic> hotel) async {
    final hotelName = hotel['name'].toString();
    final isFavorite = _favoriteHotels.contains(hotelName);
    ...
    if (isFavorite) {
      ...
      final success = await FavoritesService.addToFavorites(hotel);
      if (success) {
        setState(() {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('$hotelName added to favorites'),
  ...
  Widget _buildSearchResults() {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return Padding(
          padding: const EdgeInsets.only(bottom: 16.0),
          child: _buildHotelCardWithFavorite(hotel, CardType.horizontal),
        );
      },
    );
  }

  Widget _buildHotelCardWithFavorite(
      ...
      child: Row(
        children:
            hotels
                .map(
                  (hotel) =>
                      _buildHotelCardWithFavorite(hotel, CardType.vertical),
                )
                .toList(),
      ),
    );
    ...
    return Column(
      children:
          nearbyHotels
              .map(
                (hotel) =>
                    _buildHotelCardWithFavorite(hotel, CardType.horizontal),
              )
              .toList(),
    );
  }
